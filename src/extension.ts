...
import { OutputParser } from './compiler/parser';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    // Register commands
    const compileCommand = vscode.commands.registerCommand('cplusplusplus.compile', async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                // Create diagnostics
                const diagnostics = OutputParser.createDiagnostics(result.errors);
    ...
    if (config.cplusplusplus.enableSyntaxHighlighting) {
        ...
        const semanticTokensProvider = vscode.languages.registerDocumentSemanticTokensProvider('cpp', {
            provideDocumentSemanticTokens(document, _token) {
                ...
                function processLineSegment(segment: string, segmentOffset: number) {
                    // Track string state
